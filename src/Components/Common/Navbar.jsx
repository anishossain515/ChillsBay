...
import { UserIcon, ChevronDownIcon,ShoppingCartIcon} from "@heroicons/react/24/outline";
function NavbarArea() {
  ...
  const dropMenuItems = ['My Profile','Pay With Crypto','Cart','Contact Us','Log Out']
  ...
  return (
    ...
    >
      ...
      <NavbarContent className="space-x-8 lg:flex hidden">
        <NavbarItem className="text-Dark_Indigo text-[18px] cursor-pointer">Eat & Drink</NavbarItem>
        ...
        <NavbarItem>
          <Dropdown>
            <DropdownTrigger>
              <Button variant="none" className='text-Dark_Indigo text-[18px] cursor-pointer'>Things to do <ChevronDownIcon className="size-4 text-Dodger_Blue"/></Button>
            </DropdownTrigger>
        ...
        <Dropdown>
            <DropdownTrigger>
              <Button variant="none" className='text-Dark_Indigo text-[18px] cursor-pointer'><UserIcon className="text-Dark_Indigo size-6"/> Account</Button>
            </DropdownTrigger>
      ...
      <NavbarContent justify="end">
         ...
         {/* Toggle Button */}
         <NavbarMenuToggle aria-level={isMenuOpen ? "Close menu" : "Open menu"} className="lg:hidden"/>

         <Button color="primary" className="font-bold font-Red_Hat md:flex hidden">Contact Now</Button>
  ...
  {menuItems.map((item, index) => (
    <NavbarMenuItem key={index} className="transition-all duration-500 ease-in-out">
      {item}
    </NavbarMenuItem>
  ))}
  <NavbarMenuItem>
